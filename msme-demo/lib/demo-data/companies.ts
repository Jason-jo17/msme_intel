...
const mapStage = (stageStr?: string): CompanyStage => {
    ...
    if (s.includes('early growth')) return 'early';
    if (s.includes('scale-up') || s.includes('expansion') || s.includes('growth')) return 'growth';
    if (s.includes('mature') || s.includes('established')) return 'mature';
